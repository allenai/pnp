...
class TestMatchingCli extends AbstractCli {
  ...
  def test(examples: Seq[MatchingExample], matchingModel: MatchingModel,
    ...
    val losses = for {
      ...
      val context = PnpInferenceContext.init(compGraph)
      val dist = pnp.beamSearch(beamSize, -1, Env.init, context)
      ...
      for ((p, e) <- x.source.parts.zip(preprocessing.sourceFeatures)) {
        val v = ComputationGraph.incrementalForward(e.xy).toSeq.mkString(" ")
        println("  " + p + " " + v)
      }
      println(x.target.id)
