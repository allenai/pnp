...
class BsoTrainer(val epochs: Int, val beamSize: Int, val maxIters: Int,
  ...
  private def doExampleUpdate[A](example: PnpExample[A]): Double = {
    ...
    val queue = new BsoPnpQueue[A](beamSize, stateCost, log)
    val finished = new BsoPnpQueue[A](beamSize, stateCost, log)
    val endContinuation = new PnpEndContinuation[A]()

    val startEnv = env.setLog(log)
    // TODO(is this right?)
    val inferenceState = PnpInferenceState.init(model).addExecutionScore(stateCost)

    queue.offer(example.unconditional, env, inferenceState, null, null, env)
...
class BsoPnpQueue[A](size: Int, val stateCost: ExecutionScore, val log: LogFunction) extends PnpSearchQueue[A] {
  ...
  override def offer(value: Pnp[A], env: Env, inferenceState: PnpInferenceState, tag: Any,
    ...
    if (logProb > Double.NegativeInfinity) {
      ...
      val state = SearchState(value, nextEnv, inferenceState, tag, choice)
