...
MY_NAME=ssmn_ablated_1iter_dropout
MY_DIR=$EXPERIMENT_DIR/$MY_NAME/
MY_MODEL=$MY_DIR/model.ser
MY_FLAGS="--lstmEncode --structuralFactor --dropout 0.5"
MY_EPOCHS=1
...
mkdir -p $MY_DIR/validation_error_independent/
python $SCRIPT_DIR/visualize/visualize_loss.py $MY_DIR/validation_error_independent.json $MY_DIR/validation_error_independent/
tar cf $MY_DIR/validation_error_independent.tar $MY_DIR/validation_error_independent/
gzip -f $MY_DIR/validation_error_independent.tar
